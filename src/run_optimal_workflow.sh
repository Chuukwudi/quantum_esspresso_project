...
run_full_workflow() {
    ...
    time pw.x < ../input_files/${system}_relax.in > ${system}_relax.out 2>&1
    ...
    if [ -f "${system}_scf_on_relaxed.in" ]; then
        ...
        time pw.x < ${system}_scf_on_relaxed.in > ${system}_scf_on_relaxed.out 2>&1
...
# Package all results
tar -czf stanford_qe_optimal_results.tar.gz system*/*.out system*/*_scf_on_relaxed.in
